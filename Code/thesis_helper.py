...
class Thesis_Helper():
    ...
    def __init__(self, spacy_model='en_core_web_sm'):
        ...
        self.SGD = SGDClassifier(loss="hinge", penalty="l2", max_iter=self.iter,random_state=self.seed, n_jobs=self.processor_mode)
    ...
    def model_performance(self,x,y):
        ...
        for model in tqdm(classifiers):
            print('We are at classifier ', classifiers[model])
    ...
    def confusion_matrix(self,model,x,y):
        ...
        plot_confusion_matrix(model.fit(x_train,y_train), x_test, y_test,display_labels=['no skill', 'soft skill', 'hard skill'])
        plt.show()
